	...
	<div id="cntnr">
    	...
    	<!-- 각자의 파트는 이곳에서부터 작업하실 수 있습니다. -->
     	<form class="MpReview-arti" action="insert_review.do" method="post" onsubmit="return fnSubmit()">
     		...
     		<input type="hidden" name="rsv_id" value="${review.rsvVO.rsv_id}"/>
	...
	<script>
    	$(function(){
    		...
    		rate.each(function(){
    			...
    			$(this).find('input:nth-child(-n+' + targetScore*2 + ') + label i').css({color:'#f5d108'});
    			$(this).find('input:nth-child(n+' + targetScore*2 + ') + label i').css({color:'#dfdfdf'});
    		});
    	});
    </script>
    <script>
    	...
    	function rating(){
    		...
    		for(let i=0; i<rateLabel.length; i++){
    			rateLabel[i].onclick = function(){
    				for(let j=0; j<=i; j++){
    					change[j].setAttribute("style", "color:#f5d108;");
    				}
    				for(let k=i+1; k<rateLabel.length; k++){
    					change[k].setAttribute("style", "color:#dfdfdf;");
    				}
    			}
    		}
    	}
...
<script src="<%=request.getContextPath()%>/js/review_wrt_js.js"></script>
</html>